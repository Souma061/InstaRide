...
if (tree.size() !== 1)
  throw new Error(
    "Invariant failed: Only available drivers must be in QuadTree",
  );
...
const lock1 = await registry.acquireLock("d1", "req_100", 15_000);
console.log("    Request 100 acquired lock on d1:", lock1);
console.log("    QuadTree size after lock (should be 0):", tree.size());
if (!lock1 || tree.size() !== 0)
  throw new Error("Lock acquisition failed to pull driver from QuadTree");

// Competing request tries to acquire the same driver
const lockCompete = await registry.acquireLock("d1", "req_200", 15_000);
console.log(
  "    Competing Request 200 lock attempt (should be false):",
  lockCompete,
);
if (lockCompete)
  throw new Error("Race condition failure: Double lock allowed!");

// Competing k-NN query should find 0 available drivers
const candidates = registry.findNearbyCandidates(37.75, -122.45, 4);
console.log(
  "    Nearby candidates visible during lock (should be 0):",
  candidates.length,
);
if (candidates.length !== 0)
  throw new Error("Locked driver was visible to spatial query");

// 4. Safe Lock Release
console.log("\n[4] Testing Safe Lock Release (Wrong vs Right RequestId)...");
const wrongRelease = await registry.releaseLock("d1", "wrong_req");
console.log(
  "    Release with wrong requestId (should be false):",
  wrongRelease,
);
const correctRelease = await registry.releaseLock("d1", "req_100");
console.log("    Release with matching requestId:", correctRelease);
console.log("    QuadTree size after release (should be 1):", tree.size());
if (wrongRelease || !correctRelease || tree.size() !== 1)
  throw new Error("Safe release failed");

// 5. Trip Commitment & Completion Lifecycle
console.log("\n[5] Testing Trip Commit & Lifecycle Transitions...");
await registry.acquireLock("d1", "req_300", 15_000);
const commitOk = await registry.commitTrip("d1", "req_300");
console.log("    Trip commit result:", commitOk);
console.log(
  "    Driver status after commit (should be busy):",
  registry.getDriver("d1")?.status,
);
console.log("    QuadTree size during active trip (should be 0):", tree.size());
if (
  !commitOk ||
  registry.getDriver("d1")?.status !== "busy" ||
  tree.size() !== 0
) {
...
await registry.completeTrip("d1");
console.log(
  "    Driver status after trip completion (should be available):",
  registry.getDriver("d1")?.status,
);
console.log(
  "    QuadTree size after trip completion (should be 1):",
  tree.size(),
);
...
await registry.acquireLock("d1", "req_400", 50); // 50ms lock TTL
await new Promise((r) => setTimeout(r, 70)); // Wait for expiration

// New request should succeed because previous lock expired
const lockAfterExpire = await registry.acquireLock("d1", "req_500", 15_000);
console.log(
  "    Lock acquisition after previous expired (should be true):",
  lockAfterExpire,
);
if (!lockAfterExpire) throw new Error("Expired lock did not clean up");
await registry.releaseLock("d1", "req_500");
